...
mongoose.connect( 'mongodb+srv://admin-shubham:test123@cluster0.kzsaf.mongodb.net/usersDB' , {useNewUrlParser: true , useUnifiedTopology: true } , (err)=>{
...
app.post('/' , (req,res)=>{
  ...
  const pass = req.body.password;
   User.findOne({ name: userName , password:pass} , (err , result)=>{
        if (!err){
          if( !result ) res.render('login' , {error:'invalid username or password'})
          else {
            currentUser = result;
...
app.get( '/home' , (req,res)=>{
  ...
  const day = 'Today'; 
  res.render('list' , {name:currentUser.name  , listTitle:day , finalList: currentUser.mainList , user:currentUser});
...
app.get("/home/:customListName" ,(req,res)=>{
  ...
  let k=0;
   ...
   if (k===0)  {  //create a new list
      currentUser.lists.push(list);
      res.redirect('/home/'+custom_list_name);
   }
   else{       //render
    
    res.render('list' , { name:currentUser.name , listTitle:custom_list_name , finalList:showList , user:currentUser})
